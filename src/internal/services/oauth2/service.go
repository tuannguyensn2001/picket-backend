...
func (s *service) GetAccessTokenFromCode(ctx context.Context, code string) (string, error) {
	...
	body := map[string]string{
		...
		"code":          code,
		"client_id":     s.config.Oauth2GoogleClientId,
		"client_secret": s.config.Oauth2GoogleClientSecret,
		"redirect_uri":  s.config.ClientUrl,
	}
	...
	if val, ok := resp.Error().(*ResponseError); ok {
		log.Error().Str("error", val.Error).Send()
